            ...
            with c5:
                if campagne_sel == "ONBOARDING":
                    ...
                    recommendation = st.selectbox(
                        "Recommendation",
                        options=["", "Neutre"] + [str(i) for i in range(1, 11)],
                    )
                    feedback = st.selectbox(
                        "Feedback", options=[""] + refs["feedback"]
                    )
...
def page_historique(df, refs):
    ...
    with st.form("form_historique", clear_on_submit=False):
        ...
        campagnes_dispos = sorted({c for c in refs.get("campagnes", []) if c})
        filtre_campagnes = st.multiselect(
            "Campagne",
            options=campagnes_dispos,
            key="hist_campagnes",
        )
    ...
    if submitted:
        ...
        if filtre_campagnes:
            df_recherche = df_recherche[df_recherche["Campagne"].isin(filtre_campagnes)]
...
def page_points_de_vente(refs):
    ...
    st.markdown(
        "Ajoutez ou désactivez des points de vente. Les modifications sont "
        "enregistrées immédiatement dans la base en ligne."
    )
    ...
    if not pdv_list:
        ...
        rows_by_name = {
            str(row.get("Valeur") or "").strip().casefold(): row
            for row in refs.get("_reference_rows", [])
            if str(row.get("TypeReference") or "").strip().upper() == "POINT_DE_VENTE"
        }
        for pdv in pdv_list:
            c1, c2 = st.columns([3, 1])
            with c1:
                st.write(f"**{pdv}**")
            with c2:
                reference_row = rows_by_name.get(str(pdv).strip().casefold())
                if st.button(
                    "Désactiver",
                    key=f"del_pdv_{pdv}",
                    disabled=reference_row is None,
                    ...
                    try:
                        get_database_service().deactivate_reference(
                            reference_row.get("_item_id"),
                            etag=reference_row.get("_etag"),
                        )
                        invalider_cache_donnees()
                        st.success(f"Point de vente '{pdv}' désactivé.")
                        st.rerun()
                    except (DatabaseConfigurationError, DatabaseError) as exc:
                        st.error(f"Désactivation impossible : {exc}")
    ...
    with st.form("form_ajout_pdv"):
        nouveau_pdv = st.text_input("Nom du nouveau point de vente", placeholder="Ex: Lomé-Bé")
        ...
        if submitted:
            if not nouveau_pdv or not str(nouveau_pdv).strip():
                ...
                try:
                    result = get_database_service().add_reference(
                        "POINT_DE_VENTE", nouveau_pdv
                    )
                    invalider_cache_donnees()
                    if result == "ajoute":
                        st.success(f"Point de vente '{nouveau_pdv}' ajouté avec succès !")
                    elif result == "reactive":
                        st.warning(f"Point de vente '{nouveau_pdv}' réactivé.")
                    elif result == "existe":
                        st.warning(f"Le point de vente '{nouveau_pdv}' existe déjà dans la base.")
                    else:
                        st.error("Erreur lors de l'ajout.")
                    st.rerun()
                except (DatabaseConfigurationError, DatabaseError) as exc:
