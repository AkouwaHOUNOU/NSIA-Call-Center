...
def get_database_service():
    ...
    if not configuration.get("url") or not configuration.get("secret_key"):
        raise DatabaseConfigurationError("Base Supabase non configurée.")
...
def lire_references():
    ...
    try:
        cfg = _secret_section("database")
        if cfg.get("url") and cfg.get("secret_key"):
                ...
                try:
                    ...
                    if not items:
                        ...
                        if success_count:
                            ...
                            st.error("Aucune suppression n'a pas été confirmée par la base.")
                except Exception:
                    st.error(
